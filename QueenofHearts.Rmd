...
Suppose there are $k$ unflipped cards remaining on the board. Write 
$m = 54 - k$ for the number of weeks that the game has been played
...
x_{1}$, the conditional distribution of $X$ is 
\[ \Pr(X =
...
no x-ray vision, remember? That means 
\[ 
\Pr(\text{Somebody wins}) =
...
your tickets is selected is 
\[ 
\Pr(\text{Your ticket selected}) =
\frac{n}{N + n}.  
\] 

Given that your ticket is selected, you have
probability $\Pr(\text{Number matches}) = 1/k$ that your number
matches. Therefore, the probability that you win is 
\[ 
\Pr(\text{You
...
$1,2,\ldots,54$.  But $\Pr(L = 1)$ is 
\[ 
\Pr(L = 1) = \Pr( \text{Somebody wins
when }k\text{ is 54}) = \frac{1}{54}.  
\] 
...
\[ 
\Pr(L = 3) = \frac{53}{54}\cdot
\frac{52}{53} \cdot \frac{1}{52} = \frac{1}{54}, 
\] 
